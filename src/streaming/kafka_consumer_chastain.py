...
TIMEOUT_SECONDS: Final[float] = float(os.getenv("CONSUMER_TIMEOUT_SECONDS", "20.0"))
...
OUTPUT2_CSV: Final[Path] = OUTPUT_DIR / "Consumed_Sales2.csv"
...
def log_paths() -> None:
    ...
    log_path(LOG, "OUTPUT2_CSV", OUTPUT2_CSV)
...
def initialize_output() -> RunningStats:
    ...
    if OUTPUT2_CSV.exists():
        OUTPUT2_CSV.unlink()
    LOG.info(f"Output CSV cleared: {OUTPUT2_CSV.name}")
...
def save_artifacts(stats: RunningStats) -> None:
    ...
    log_path(LOG, "WROTE OUTPUT2_CSV", OUTPUT2_CSV)
